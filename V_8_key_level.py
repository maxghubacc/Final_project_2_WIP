...
def main():
    ...
    while True:
        ...
        if state == "PLAYING":
            ...
            # Determine what tile we'd move into (if in bounds)
            blocked = False
            ...
            if not in_bounds(new_cell, rows, cols):
                ...
                # Locked exit gate in Level 4 blocks until key collected
            ...
            if blocked:
                ...
                # Win condition:
                # - Normal levels: touching G wins
                # - Level 4: only wins if has_key
                if new_head == goal_px:
                    if current_level != KEY_LEVEL_INDEX or has_key:
                        state = "WIN"
                    else:
                        # Shouldn't happen because gate blocks, but keep safe:
                        need_key_popup_ticks = 30
        ...
        # Goal (Exit)
        # In Level 4, show exit as "locked" until key collected
