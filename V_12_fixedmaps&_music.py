...
AUDIO_VOLUME_MUSIC = 0.22
AUDIO_VOLUME_SFX = 0.45
...
PORTAL_PAUSE_TICKS = FPS  # 1 second pause after teleport
...
# ---------- Procedural Audio ----------
def tone_samples(freq, duration, volume=0.35, wave_type="square"):
    n_samples = int(AUDIO_SAMPLE_RATE * duration)
    ...
    for i in range(n_samples):
        ...
        if wave_type == "sine":
            ...
            sample = 2 * abs(2 * ((freq * t) % 1) - 1) - 1
        else:
            sample = 1.0 if math.sin(2 * math.pi * freq * t) >= 0 else -1.0

        fade = 1.0
        fade_len = min(200, n_samples // 6)
        if i < fade_len:
            fade = i / max(1, fade_len)
        elif i > n_samples - fade_len:
            fade = (n_samples - i) / max(1, fade_len)

        buf.append(int(amplitude * sample * fade))
    return buf

def silence_samples(duration):
    return array("h", [0] * int(AUDIO_SAMPLE_RATE * duration))
...
def build_music_loop():
    melody = [
        (-12, 0.18), (-5, 0.18), (-8, 0.18), (-12, 0.18),
        (-10, 0.18), (-3, 0.18), (-7, 0.18), (-10, 0.18),
        (-12, 0.18), (-5, 0.18), (-8, 0.18), (-12, 0.18),
        (-7, 0.18), (-3, 0.18), (-5, 0.18), (-8, 0.18),
    ]

    bass = [
        (-24, 0.36), (-20, 0.36), (-24, 0.36), (-19, 0.36),
        (-24, 0.36), (-20, 0.36), (-22, 0.36), (-19, 0.36),
    ]

    total = array("h")
    bass_index = 0
    bass_time_left = 0.0
    current_bass = silence_samples(0)

    for semi, dur in melody:
        melody_chunk = tone_samples(note_freq(semi), dur, volume=0.14, wave_type="square")

        if bass_time_left <= 0:
            bsemi, bdur = bass[bass_index % len(bass)]
            current_bass = tone_samples(note_freq(bsemi), bdur, volume=0.10, wave_type="triangle")
            bass_time_left = bdur
            bass_index += 1

        bass_chunk_len = len(melody_chunk)
        bass_chunk = current_bass[:bass_chunk_len]
        if len(bass_chunk) < bass_chunk_len:
            bass_chunk.extend([0] * (bass_chunk_len - len(bass_chunk)))
        current_bass = current_bass[bass_chunk_len:]
        bass_time_left -= dur

        mixed = array("h")
        for m, b in zip(melody_chunk, bass_chunk):
            val = max(-32767, min(32767, m + b))
            mixed.append(val)
        total.extend(mixed)
        total.extend(silence_samples(0.02))

    snd = pygame.mixer.Sound(buffer=total.tobytes())
    snd.set_volume(AUDIO_VOLUME_MUSIC)
    return snd

def build_ding_sound():
    parts = array("h")
    parts.extend(tone_samples(880, 0.06, volume=0.35, wave_type="square"))
    parts.extend(tone_samples(1320, 0.08, volume=0.32, wave_type="square"))
    parts.extend(tone_samples(1760, 0.10, volume=0.25, wave_type="sine"))
    snd = pygame.mixer.Sound(buffer=parts.tobytes())
    snd.set_volume(AUDIO_VOLUME_SFX)
    return snd

def build_sad_sound():
    parts = array("h")
    for semi, dur in [(-5, 0.12), (-8, 0.12), (-12, 0.18)]:
        parts.extend(tone_samples(note_freq(semi), dur, volume=0.30, wave_type="triangle"))
        parts.extend(silence_samples(0.01))
    snd = pygame.mixer.Sound(buffer=parts.tobytes())
    snd.set_volume(AUDIO_VOLUME_SFX)
    return snd
...
def build_win_sound():
    parts = array("h")
    notes = [0, 4, 7, 12, 16]
    durations = [0.10, 0.10, 0.12, 0.12, 0.22]
    for semi, dur in zip(notes, durations):
        parts.extend(tone_samples(note_freq(semi), dur, volume=0.33, wave_type="square"))
        parts.extend(silence_samples(0.015))
    snd = pygame.mixer.Sound(buffer=parts.tobytes())
    snd.set_volume(AUDIO_VOLUME_SFX)
    return snd

def build_portal_sound():
    parts = array("h")
    # sci-fi swoop up then down
    sweep_steps = 12
    for i in range(sweep_steps):
        f = 500 + i * 70
        parts.extend(tone_samples(f, 0.015, volume=0.22, wave_type="sine"))
    for i in range(sweep_steps):
        f = 1340 - i * 60
        parts.extend(tone_samples(f, 0.012, volume=0.18, wave_type="triangle"))
    snd = pygame.mixer.Sound(buffer=parts.tobytes())
    snd.set_volume(AUDIO_VOLUME_SFX)
    return snd
...
def build_key_sound():
    parts = array("h")
    for f, d in [(740, 0.05), (988, 0.05), (1480, 0.08)]:
        parts.extend(tone_samples(f, d, volume=0.28, wave_type="square"))
        parts.extend(silence_samples(0.01))
    snd = pygame.mixer.Sound(buffer=parts.tobytes())
    snd.set_volume(AUDIO_VOLUME_SFX)
    return snd

def build_locked_sound():
    parts = array("h")
    parts.extend(tone_samples(180, 0.05, volume=0.28, wave_type="square"))
    parts.extend(tone_samples(140, 0.08, volume=0.24, wave_type="triangle"))
    snd = pygame.mixer.Sound(buffer=parts.tobytes())
    snd.set_volume(AUDIO_VOLUME_SFX)
    return snd
...
def main():
    ...
    # Audio
    music_sound = build_music_loop()
    ding_sound = build_ding_sound()
    sad_sound = build_sad_sound()
    win_sound = build_win_sound()
    portal_sound = build_portal_sound()
    key_sound = build_key_sound()
    locked_sound = build_locked_sound()
    ...
    portal_pause_ticks = 0

    def lose_life():
        nonlocal lives, hit_flash
        lives -= 1
        hit_flash = 8
        sfx_channel.play(sad_sound)
    ...
    def load_level(level_index: int):
        ...
        nonlocal portals_active, portal_pairs, portal_popup_ticks, portal_popup_text, portal_popup_pos_px, portal_pause_ticks
    ...
    while True:
        ...
        if portal_pause_ticks > 0:
            portal_pause_ticks -= 1
            ...
            if portal_pause_ticks == 0:
                ...
                if blocked:
                    ...
                    if portals_active and head_cell in portal_pairs:
                        ...
                        direction = (0, 0)
                        pending_dir = (0, 0)
                        portal_pause_ticks = PORTAL_PAUSE_TICKS
                        sfx_channel.play(portal_sound)
                    ...
                    if current_level == KEY_LEVEL_INDEX:
                        ...
                        if head_cell in key_cells and head_cell not in collected_keys:
                            ...
                            sfx_channel.play(key_sound)

                    if snake[0] == goal_px:
                        if current_level != KEY_LEVEL_INDEX or has_key:
                            state = "WIN"
                            sfx_channel.play(win_sound)
                        else:
                            need_key_popup_ticks = 14
                            sfx_channel.play(locked_sound)
        ...
        if portal_pause_ticks > 0:
            draw_glow_text(
                ...
                WHITE,
                PURPLE_NEON,
                (screen.get_width() // 2, HUD_HEIGHT + 28),
            )
        ...
        if current_level == KEY_LEVEL_INDEX:
            draw_text(screen, f"KEY: {'YES' if has_key else 'NO'}", 20, KEY_COLOR, (w - 180, 28))
        elif current_level == PORTAL_LEVEL_INDEX:
            draw_text(screen, "PORTALS: 2 SETS", 18, WHITE, (w - 190, 28))

        draw_text(screen, "M: MENU", 18, HUD_TEXT, (w - 65, 28))
