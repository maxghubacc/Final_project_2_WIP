...
# Characters:
# # = wall, S = start, G = goal, . = empty, E = enemy spawn, B = bonus
    ...
    while True:
        ...
        if state == "PLAYING":
            ...
            # Self collision (only matters if you make snake longer later)
                ...
                # Bonus pickup (Level 3 only, tile 'B')
                if current_level == BONUS_LEVEL_INDEX:
        ...
        if state == "MENU":
            ...
            draw_text(screen, "3 - Level 3 (Enemies + Bonus)", 28, WHITE, (w // 2, h // 2 + 65))
        ...
        # Bonus dots (Level 3 only) - draw before fog (level 2 fog won't have bonuses anyway)
        if current_level == BONUS_LEVEL_INDEX:
